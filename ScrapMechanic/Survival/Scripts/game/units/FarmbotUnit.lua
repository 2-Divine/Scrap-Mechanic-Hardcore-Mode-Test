...
function FarmbotUnit.server_onCreate( self )
	...
	self.rangedAttack.projectile = "pesticide"
	...
	if self.currentState then
		...
		if self.currentState == self.combatFollowAttackState or self.currentState == self.combatChaseAttackState or self.currentState == self.pathingState then
			...
			if self.stompTimer:done() then
				...
				sm.melee.meleeAttack( "FarmbotStep", 3, self.unit.character.worldPosition + self.unit.character.direction * 0.875 - sm.vec3.new( 0, 0, self.unit.character:getHeight() * 0.375 ), self.unit.character.direction * 1.75, self.unit, 0, 4000 )
			end
		else
			self.stompTimer:reset()
		end
...
function FarmbotUnit.server_onUnitUpdate( self, dt )
	...
	if self.lastTargetPosition then
		...
		inFireRange = desiredDirection:length() <= FireRange
		inCombatFollowRange = desiredDirection:length() <= CombatFollowRange
		inCombatAttackRange = desiredDirection:length() <= CombatAttackRange
		inSprintRange = desiredDirection:length() >= SprintRange
...
function FarmbotUnit.server_onProjectile( self, hitPos, hitTime, hitVelocity, projectileName, attacker, damage )
...
function FarmbotUnit.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal  )	
	...
	if damage > 0 then
		print("'FarmbotUnit' took collision damage")
...
function FarmbotUnit.sv_takeDamage( self, damage, impact, hitPos )
	if self.saved.stats.hp > 0 then
		...
		if self.saved.stats.hp <= 0 then
			startTumble( self, self.destroyedEventState.time * 40, self.destroyedEventState )
		else
...
function FarmbotUnit.sv_onDeath( self )
	...
	if not self.destroyed then
		...
		-- Create explosion
		sm.physics.explode( character.worldPosition, 7, 2, 6, 25, "Farmbot - Destroyed" )
		self:sv_spawnParts()
...
function FarmbotUnit.sv_spawnParts( self )
	local character = self.unit:getCharacter()
	...
	if lookDirection:length() >= FLT_EPSILON then
		...
		bodyRot = sm.vec3.getRotation( sm.vec3.new( 0, 1, 0 ), lookDirection  ) --Turn farmbot parts sideways
	end
	...
	local middleShape= middleBody:createPart( obj_robotparts_farmbotpart_drill, sm.vec3.new( 0, -3, -7 ), sm.vec3.new( 0, 1, 0 ), sm.vec3.new( -1, 0, 0 ), true )

	local leftArmBody = sm.body.createBody( bodyPos, bodyRot, true )
	local leftArmShape = leftArmBody:createPart( obj_robotparts_farmbotpart_cannonarm, sm.vec3.new( -9, -1, 1 ), sm.vec3.new( 0, -1, 0 ), sm.vec3.new( 1, 0, 0 ), true )
	
	local rightArmBody = sm.body.createBody( bodyPos, bodyRot, true )
	local rightArmShape = rightArmBody:createPart( obj_robotparts_farmbotpart_scytharm, sm.vec3.new( 7, -1, -1 ), sm.vec3.new( 0, 1, 0 ), sm.vec3.new( -1, 0, 0 ), true )
end
