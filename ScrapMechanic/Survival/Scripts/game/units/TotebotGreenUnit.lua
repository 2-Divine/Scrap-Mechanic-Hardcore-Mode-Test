...
function TotebotGreenUnit.server_onCreate( self )
	...
	if self.saved.stats == nil then
		self.saved.stats = { hp = 15, maxhp = 15 }
	end
...
function TotebotGreenUnit.server_onUnitUpdate( self, dt )
	...
	if self.lastTargetPosition then
		...
		inCombatApproachRange = fromToTarget:length() <= CombatApproachRange
		inCombatAttackRange = fromToTarget:length() <= CombatAttackRange
...
function TotebotGreenUnit.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal )
	...
	if damage > 0 then
		print("'TotebotGreenUnit' took collision damage")
...
function TotebotGreenUnit.sv_onDeath( self, impact )
	...
	if not self.destroyed then
		sm.effect.playEffect( "ToteBot - DestroyedParts", character.worldPosition, nil, nil, nil, { Color = sm.color.new( "49642d" ) } )
...
function TotebotGreenUnit.sv_spawnParts( self, impact )
	...
	if math.random( 1, 5 ) == 1 then
		...
		headShape.color = sm.color.new( "49642d" )
	end
	...
	middleShape.color = sm.color.new( "49642d" )
	...
	legShape.color = sm.color.new( "49642d" )
