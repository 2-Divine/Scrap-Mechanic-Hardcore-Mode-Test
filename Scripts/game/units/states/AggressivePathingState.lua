...
AggressivePathingState = class( nil )

-- State specific functions

function AggressivePathingState.sv_onCreate( self, unit )
	self.unit = unit
	self.tolerance = 1.0
...
function AggressivePathingState.sv_onDestroy( self )
	self:sv_debugDrawCleanup()
	self.followPathState:sv_onDestroy()
end

function AggressivePathingState.sv_setMovementType( self, movementType )
	self.followPathState:sv_setMovementType( movementType )
end

function AggressivePathingState.sv_setTolerance( self, tolerance )
	self.tolerance = tolerance
	self.followPathState.tolerance = tolerance
end

function AggressivePathingState.sv_findPath( self, toPosition )
	local path = sm.pathfinder.getPath( self.unit.character, toPosition, false )
	print( path )
	...
	print( "AggressivePathingState found a path with", #self.mainPath, "nodes" )

	return #self.mainPath > 1
end

function AggressivePathingState.sv_getBreachParams( self )
...
function AggressivePathingState.start( self )
	self:sv_debugText( "AggressivePathingState started" )
	if not ( #self.mainPath > 1 ) then
		print( "AggressivePathingState for unit", self.unit.id, "does not have enough nodes to start" )
		return
	end
...
function AggressivePathingState.stop( self )
	self.started = false
	...
	self:sv_debugText( "AggressivePathingState stopped" )
end

function AggressivePathingState.onFixedUpdate( self, timeStep )
	...
	if not self.done and self.followPathState.path == self.mainPath then
		...
		if ( self.unit.character.worldPosition - closestLinePos ):length() > self.tolerance or progress < self.bestProgess - 1.0 / segmentLength then
			...
			elseif self.backtrackTimer then
				...
				if self.backtrackTimer > 0.5 then --TWEAKABLE
					local patience = 4
...
function AggressivePathingState.onUnitUpdate( self, dt )
	if not self.started then return end
	
	self.followPathState:onUnitUpdate( dt )
end

function AggressivePathingState.isDone( self )
	if self.done then
...
function AggressivePathingState.getMovementDirection( self )
	return self.followPathState:getMovementDirection()
end

function AggressivePathingState.getFacingDirection( self )
	return self.followPathState:getFacingDirection()
end

function AggressivePathingState.getMovementType( self )
	return self.followPathState:getMovementType()
end

-- Private

function AggressivePathingState.sv_pushPath( self, path )
...
function AggressivePathingState.sv_popPath( self )
...
function AggressivePathingState.sv_breach( self )
...
function AggressivePathingState.sv_debugDrawCleanup( self )
	sm.debugDraw.removeSphere( "u_"..self.unit.id.."_closestLinePos" )
	
	self:sv_clearBacktrackDebugDraw()
end

function AggressivePathingState.sv_clearBacktrackDebugDraw( self )
...
function AggressivePathingState.sv_debugText( self, text )
