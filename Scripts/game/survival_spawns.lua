...
local function CreateUnit( tag, node, cell )
	...
	local unit
	if tag == "HAYBOT" then
		unit = sm.unit.createUnit( unit_haybot, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400  } )
	elseif tag == "TOTEBOT_GREEN" then
		unit = sm.unit.createUnit( unit_totebot_green, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
	elseif tag == "FARMBOT" then
		unit = sm.unit.createUnit( unit_farmbot, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
	elseif tag == "TAPEBOT" then
		unit = sm.unit.createUnit( g_tapebots[math.random( 1, #g_tapebots )], node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
	elseif tag == "WOC" then
		unit = sm.unit.createUnit( unit_woc, node.position, toYaw( node.rotation ), { tetherPoint = node.position, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
...
local function SpawnUnits( cell, nodes, spawnMax, storage, tag )
	...
	for _, node in ipairs( nodes ) do		
		...
		if node.params and node.params.guaranteed then
			...
			table.insert( storage.units, CreateUnit( tag, node, cell ) )
	...
	for _, node in ipairs( nodes ) do
		...
		if cell.isStartArea then
			if guaranteed or math.random( 100 ) <= StartareaLootcrateSpawnChance[cell.isPoi] then
				local harvestable = sm.harvestable.create( hvs_lootcrate, node.position, node.rotation )
			...
			if guaranteed or math.random( 100 ) <= LootcrateSpawnChance[cell.isPoi] then
				if math.random( 100 ) <= LootcrateUpgradeChance[cell.isPoi] then
					local harvestable = sm.harvestable.create( hvs_lootcrateepic, node.position, node.rotation )
...
local SpawnDescriptions = {
	...
	["SEED_SPAWN"] = {
		...
		ticksBetweenRespawns = DaysInTicks( 100 ),
